// pages/services/store-setup-and-branding-services/[platform].js

import { useRouter } from 'next/router';
import Head from 'next/head';
import { storeSetupPages } from '@/src/data/servicePages/storeSetup';
...
export default function PlatformServicePage() {
  ...
  const pageData = platform ? storeSetupPages[platform.toLowerCase()] : null;
  ...
  const canonicalUrl = `https://technovitasolution.in/services/store-setup-and-branding-services/${platform}`;
      ...
      <Head>
        ...
        <Breadcrumb
          items={[
            ...
            { name: platform, href: `/services/store-setup-and-branding-services/${platform}` },
          ]}
...
export async function getStaticPaths() {
  const paths = Object.keys(storeSetupPages).map(platform => ({
    params: { platform }
  }));
...
export async function getStaticProps({ params }) {
  ...
  if (!platform || !storeSetupPages[platform]) {
    return {
      notFound: true
    };
  }
  ...
  return {
    props: {
      pageData: storeSetupPages[platform]
    },
    revalidate: 86400
  };
}
