// pages/services/brand-documentation/[platform].js

import { useRouter } from 'next/router';
import Head from 'next/head';
import { brandDocumentation } from '@/src/data/servicePages/brandDocumentation';
...
export default function PlatformServicePage() {
  ...
  const pageData = platform ? brandDocumentation[platform.toLowerCase()] : null;
  ...
  const canonicalUrl = `https://technovitasolution.in/services/brand-documentation/${platform}`;
      ...
      <Head>
        ...
        <Breadcrumb
          items={[
            ...
            { name: platform, href: `/services/brand-documentation/${platform}` },
          ]}
...
export async function getStaticPaths() {
  const paths = Object.keys(brandDocumentation).map(platform => ({
    params: { platform }
  }));
...
export async function getStaticProps({ params }) {
  ...
  if (!platform || !brandDocumentation[platform]) {
    return {
      notFound: true
    };
  }
  ...
  return {
    props: {
      pageData: brandDocumentation[platform]
    },
    revalidate: 86400
  };
}
