...
const AccountManagementServices = () => {
  ...
  useEffect(() => {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            setVisibleServices(prev => [...prev, entry.target.dataset.platform]);
          }
        });
      },
      { threshold: 0.1 }
    ...
    document.querySelectorAll('[data-platform]').forEach((el) => observer.observe(el));
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {platforms.map((platform, index) => (
            <div key={platform.name} data-platform={platform.name}>
              <ServiceCard 
                platform={platform}
                isVisible={visibleServices.includes(platform.name)}
              />
            </div>
          ))}
        </div>
