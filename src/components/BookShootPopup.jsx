...
const BookShootPopup = ({ isOpen, onClose, shootType }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      const res = await fetch("/api/book-shoot", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...form, shootType }),
      });
      if (res.ok) {
        setSuccess(true);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-2 sm:p-4 overflow-y-auto">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl relative p-6 mx-auto">
        ...
        <div className="text-center mb-4">
          <h2 className="text-2xl font-bold text-blue-700 mb-2">Book Your Product Shoot</h2>
          <p className="text-orange-500 font-semibold mb-2">Special Discount on Your First Shoot!</p>
          <p className="text-gray-600 text-sm mb-2">Shoot Type: <span className="font-bold text-black">{shootType}</span></p>
        </div>
        {success ? (
